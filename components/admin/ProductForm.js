...
export default function ProductForm({ product = null, onSubmit, onCancel, loading = false }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await onSubmit({ ...form, price: parseFloat(form.price), stock: parseInt(form.stock) || 0, image_url })
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {imagePreview ? (
            ...
            <>
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-steel-400 mt-1">PNG, JPG, WebP up to 5MB</p>
              </div>
            </>
          )}
          {uploading && (
      ...
      <div>
        ...
        <textarea
          ...
          className={cn('input-field resize-none', errors.description && 'border-red-500')}
        />
        {errors.description && <p className="text-red-500 text-xs mt-1">{errors.description}</p>}
        <p className="text-xs text-steel-400 mt-1">{form.description.length}/2000</p>
      </div>
      ...
      <label className="flex items-center gap-3 cursor-pointer">
        <div className="relative">
          ...
          <div className={cn(
            ...
            form.is_featured ? 'bg-brand-500' : 'bg-steel-300 dark:bg-steel-600'
          )}>
            <div className={cn(
      ...
      <div className="flex gap-3 pt-2">
        ...
        {onCancel && (
          <Button type="button" variant="ghost" onClick={onCancel} className="flex-shrink-0">
            Cancel
          </Button>
        )}
      </div>
